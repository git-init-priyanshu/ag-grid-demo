...
>({
  ...
  const onSaveUpdates = useCallback(async () => {
    ...
    for (const [rowId, rowData] of editedRows.entries()) {
      ...
      if (String(rowId).startsWith("temp_")) {
        // Don't send server-managed or FE-only fields to the BE
        delete dataToSend.generatedId; // NOTE: We can do better than this man
        delete dataToSend[uniqueIdentifier];
        delete dataToSend.createdAt; // auto-generated by Prisma (@default(now()))
        delete dataToSend.userId;    // injected server-side from the session
        addUpdates.push(dataToSend);
        orderedTempIds.push(String(rowId)); // track order
      } else {
    ...
    // Handle add mutation result: correlate by position (response[i] → orderedTempIds[i])
    ...
    } else if (addResult.success && !addResult.skipped) {
      // Swap temp rows with BE-generated rows using positional correlation
      const beRows = (bulkAddMutation!.data ?? []) as T[];
      const tempRowsToRemove: T[] = [];
      const rowsToAdd: T[] = [];

      beRows.forEach((beRow, i) => {
        const tempId = orderedTempIds[i];
        if (!tempId) return;
        rowsToAdd.push(beRow);
  ...
  const getRowId = useCallback((params: GetRowIdParams) => {
    const id = params.data[uniqueIdentifier] ?? `temp_${uuidv4()}`;
    params.data.generatedId = id;
